...
def test_admin_endpoints_unauthorized(client, auth_headers):
    # Standard user
    headers = auth_headers(client, "normie", "password", is_admin=False)
...
def test_admin_get_users(client, auth_headers):
    headers = auth_headers(client, "adminuser", "password", is_admin=True)
    resp = client.get("/admin/users", headers=headers)
    assert resp.status_code == 200
    assert isinstance(resp.json(), list)

def test_admin_promote_user(client, auth_headers):
    admin_headers = auth_headers(client, "admin2", "password", is_admin=True)
    # Create normal user
...
def test_admin_delete_user(client, auth_headers):
    admin_headers = auth_headers(client, "admin3", "password", is_admin=True)
...
def test_admin_delete_self(client, auth_headers):
    admin_headers = auth_headers(client, "admin4", "password", is_admin=True)
    users = client.get("/admin/users", headers=admin_headers).json()
...
def test_admin_stats_and_logs(client, auth_headers):
    admin_headers = auth_headers(client, "admin5", "password", is_admin=True)
